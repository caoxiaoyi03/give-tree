...
class GiveTreeNode {
  ...
  /**
   * Remove data entries from the node. This is for local-only trees that
  ...
  /**
   * Remove data entries from the cache. This is for non-local trees that does
   * not support editing and the content of the tree will not be altered.
   *
   * @abstract
   *
   * Data entries with the same start (and end values if exists) will be
   ...
   * call `this._root = this._root.restructure()` after removing nodes.
   *
   * @param {ChromRegion} data The data
   *    entry being removed.
   * @param {object} props Additional properties being
   *    passed onto nodes.
   * @param {DataEntryCallback} [props.dataCallback] The callback
