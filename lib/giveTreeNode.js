...
class GiveTreeNode {
   ...
   */
  remove (data, exactMatch, convertTo, props, ...args) {
  ...
  static _traverseData (
    data, currIndex, chrRange, critFunc, callback, ...args
  ) {
    // Helper function: find `entries` in `data` that returns `true` with
    //    `critFunc.call(thisVarCriteria, entry)`, call `callback` on
    //    `entry` if `callback` exists and advance `currIndex`.
    ...
    while (
      currIndex < data.length && critFunc(data[currIndex], chrRange, ...args)
    ) {
      if (typeof callback === 'function') {
        callback(data[currIndex], chrRange, ...args)
      }
      currIndex++
    }
    return currIndex
   ...
   * @param  {DataEntryCallback} callback The callback function whose return
   *    value can be evaluated as a boolean value to determine
   *    whether the call shall continue (if `breakOnFalse === true`).
   * @param  {DataEntryCallback} [filter] A filter function that returns whether
   ...
   * @param  {ChromRegion} chrRange The chromosomal range
   *    to traverse.
   * @param  {DataEntryCallback} callback
   *    The callback function, takes a `ChromRegion` object as its first
   *    parameter and returns something that can be evaluated as a boolean
   *    value to determine whether the call shall continue
   *    (if `breakOnFalse === true`).
   * @param  {DataEntryCallback} [filter]
   *    A filter function that takes a `ChromRegion` object as its first
   *    parameter and returns whether the region should be included in traverse.
   *
   *    Filtered entries will not break the traverse since `callback` will not
