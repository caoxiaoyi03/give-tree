...
class GiveTree {
  ...
  _insertSingleRange (data, chrRange, props, ...args) {
    ...
    if (!chrRange || !chrRange.chr || chrRange.chr === this.chr) {
      ...
      this._preInsertionOp(data, chrRange, props, ...args)
      this._root = this._root.insert(data, chrRange, props, ...args)
    }
  }
   ...
   * @memberof GiveTree#
   */
  _preInsertionOp (data, chrRange, props, ...args) {
    if (typeof this._LeafNodeCtor.preInsertionOp === 'function') {
      props.doNotWither = true
      return this._LeafNodeCtor.preInsertionOp(
        data, chrRange, this, props, ...args)
    }
  }
  ...
  /**
   * Removing an array of single data entries. Notice that the behavior of this
   * function will not be the same depending on whether the tree is local only
   * (indicated by `this.localOnly`)
   *
   * *   If the tree is local only, deletion will take actual data entries and
   *     remove them from the tree (replace by `false` if needed)
   * *   If the tree is not local only, deletion will take ranges within the
   *     tree and replace them with `null` (to be replenished later).
   *
   * @param {Array<ChromRegion>|ChromRegion} data The data that needs to be
   *    removed.
  ...
  remove (data, props, ...args) {
    ...
    data.forEach(dataEntry => this._root.remove(dataEntry, props, ...args))
  ...
  traverse (chrRange, props, ...args) {
    if (!props || (typeof props.nodeCallback !== 'function' &&
      typeof props.dataCallback !== 'function'
    )) {
      throw new Error('At least one type of callback is needed!')
    }
    if (!chrRange || !chrRange.chr || chrRange.chr === this.chr) {
   ...
   */
  clear (convertTo) {
    return this._root.clear(convertTo)
  }
}
