...
class GiveTree {
  ...
  /**
   * Pre-insertion operation to ensure data consistency.
   * The typical implementation will merge all data overlapping `chrRange.start`
  ...
  _preInsertionOp (data, chrRange, props) {
    ...
    let _preInsContinuedList = props.continuedList.concat(
      data.filter(region =>
        (region.start < chrRange.start && region.overlaps(chrRange))
      )
    ).sort(ChromRegion.compare)
    // 2. Replace `props.continuedList`
    let preInsertionRange = this.coveringRange.getMinus(chrRange)[0]
    if (preInsertionRange) {
      /**
       * Helper callback function
       * @type {DataEntryCallback}
       * @param {ChromRegion} dataEntry
       * @param {ChromRegion} insertionRange
       * @param {Array<ChromRegion>} preInsContinuedList Pre-operation
       *  `continuedList`
       * @param {Array<ChromRegion>} continuedList Post-processed
       *  `continuedList` of previous regions.
       */
      let callback =
        (dataEntry, insertionRange, preInsContinuedList, continuedList) => {

        }
      this._traverse(preInsertionRange, callback
        , null)
    }
  }
   ...
   * @param {DataEntryCallback} [props.callback] The callback function to be
   *    used when inserting.
   ...
   * @param  {DataEntryCallback} [callback] The callback function to be used
   *    when the data entry is/entries are being removed.
   * @param  {object} [props] Additional properties being passed onto
   *    nodes
   * @param {...object} [args] Additional arguments that may be passed to all
   *    callbacks.
   */
  remove (data, exactMatch, convertTo, callback, props, ...args) {
    if (convertTo === undefined) {
      convertTo = null
    }
    props = props || {}
    props.callback = callback
    this._root.remove(data, exactMatch, convertTo, props, ...args)
   ...
   *    range.
   * @param {DataEntryCallback} callback The callback function to be used on all
   *    overlapping data entries (that pass `filter` if it exists).
   * @param {DataEntryCallback} [filter] The filter function to be used, return
   *    `false` to exclude the entry from being called with `callback`.
   ...
   * @param {DataEntryCallback} callback The callback function to be used on all
   *    overlapping data entries (that pass `filter` if it exists).
   * @param {DataEntryCallback} [filter] The filter function to be used, that
   *    returns `false` to exclude the entry from being called with `callback`.
   * @param {boolean} [breakOnFalse=false] Whether the traversing should break
   *    if `false` has been returned from `callback`
   * @param {object} [props] Additional properties being passed onto
   *    nodes
