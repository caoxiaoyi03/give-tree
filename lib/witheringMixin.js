...
/**
 * @typedef {import('./giveNonLeafNode')} GiveNonLeafNode
 *
 * Withering nodes.
 *
 ...
 *
 * @param {function(new:GiveNonLeafNode)} Base
 * @mixin WitheringMixin
 * @class
 * @extends GiveNonLeafNode
 * @returns {function(new)}
 */
const WitheringMixin = Base => class WitherableNode extends Base {
  /**
   * @constructor
  ...
  constructor (props) {
    ...
    if ((props && props._currGen) || (this.tree && this.tree.lifeSpan)) {
      /**
       * Last updated generation of this node. This value will be updated
       * whenever `.insert()` or `.traverse()` is called so that active branches
       * won't wither easily.
       *
   ...
   *
   * @param {number} generationToComp the generation to be compared
  ...
  wither () {
    ...
    // then call `this.remove` on all children marked.
