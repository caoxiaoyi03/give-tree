...
class DataNode extends GiveTreeNode {
   ...
   * @param {DataEntryCallback} [props.callback] The callback
  ...
  insert (data, chrRange, props, ...args) {
    ...
    currIndex = this.constructor._traverseData(data, currIndex, chrRange,
      dataEntry => dataEntry.start < this.start, props.callback, ...args)
  ...
  _updateContinuedList (extList, throwIfNotConsistent) {
    if (extList) {
      extList = extList.filter(entry => (entry.end > this.start))
   ...
   * @param {Array<ChromRegion>} preInsContinuedList List before sync.
   * @param {Array<ChromRegion>} continuedList List after sync.
   */
  static _updateExtContinuedList (
    dataEntry, syncRange, preSyncContinuedList, continuedList
  ) {
    if (preSyncContinuedList.length) {
      if (preSyncContinuedList[0].start < dataEntry.start()) {
        throw new Error('ContinuedList inconsistent: entry ' +
          preSyncContinuedList[0].toString() + ' does not exist before ' +
          'existing entry ' + dataEntry.toString() + '!')
      }
      if (dataEntry.equalTo(preSyncContinuedList[0])) {
        continuedList.push(dataEntry)
        preSyncContinuedList.shift()
        syncRange.start = preSyncContinuedList.length
          ? Math.min(preSyncContinuedList[0].start, syncRange.end)
          : syncRange.end
      }
    }
  }
   ...
   */
  static preInsertionOp (data, insertRange, tree, props) {
    // 1. Merge all proper data to `props.continuedList`
    let _preInsContinuedList = props.continuedList.concat(
      data.filter(region =>
        (region.start < insertRange.start && region.overlaps(insertRange))
      )
    ).sort(ChromRegion.compare)
    ...
    if (preInsertionRange) {
      tree.traverse(preInsertionRange, this._updateExtContinuedList.bind(this),
        null, false, { notFirstCall: true }, _preInsContinuedList,
        props.continuedList)
    }
  }
}
