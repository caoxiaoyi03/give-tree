...
class GiveNonLeafNode extends GiveTreeNode {
  ...
  removeData (data, props, ...args) {
    ...
    // Rationale: remove data one by one and remove the empty nodes if possible
    // By merging.
  ...
  get isEmpty () {
    return this.childNum <= 0 || (this.childNum === 1 &&
      (this.values[0] === false ||
        !!(this.values[0] && ((this.reverseDepth || !this.tree.localOnly) ? this.values[0].isEmpty : this.values[0].hasEmptyStart))))
  }
}
