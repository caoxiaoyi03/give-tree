 ...
 */
class BasicSampleNode extends GiveNonLeafNode {
  ...
  _addLeafRecords (data, chrRange, props, ...args) {
    ...
    while (chrRange.start < chrRange.end) {
      ...
      if (
        ...
        newChild = this.values[currIndex] || new props.LeafNodeCtor({
          start: this.keys[currIndex],
          end: this.keys[currIndex + 1]
        })
        newChild.insert(data, chrRange, props, ...args)
        // if (newChild.isEmpty) {
        //   newChild = false
        // }
      }
      ...
      if (nextStart < this.keys[currIndex + 1]) {
        this._splitChild(currIndex, nextStart)
      }

      this.values[currIndex] = newChild
      if (this._mergeChild(currIndex, false, true)) {
        currIndex--
      }

      // Shrink `chrRange` to unprocessed range
      chrRange.start = nextStart
    }
 ...
 * @memberof module:SampleNodes.BasicSampleNode
 */
BasicSampleNode.restructuringRequired = true

class RestrSampleNode extends BasicSampleNode {
  /**
 ...
 * @property {boolean} restructuringRequired - Whether restructuring is needed
 *    for this class of node
 * @memberof module:SampleNodes.RestrSampleNode
 */
RestrSampleNode.restructuringRequired = true

/**
 * @exports SampleNodes.BasicSampleNode
 * @exports SampleNodes.RestrSampleNode
 */
module.exports.BasicSampleNode = BasicSampleNode
module.exports.RestrSampleNode = RestrSampleNode
