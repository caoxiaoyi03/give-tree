 ...
 *
 */
class PineNode extends GiveNonLeafNode {
  /**
    ...
    if (typeof this.tree._SummaryCtor === 'function') {
      ...
      if (newSummary instanceof this.tree._SummaryCtor) {
        ...
        if (newSummary) {
          ...
          _verbConsole.info(newSummary + ' is not a correct summary ' +
   ...
   *    See `GiveNonLeafNode.prototype._restructure` for details.
   */
  ...
  _addLeafRecords (data, chrRange, props) {
    ...
    while (chrRange.start < chrRange.end) {
      ...
      props.dataIndex = _traverseData(data, props.dataIndex,
  ...
  remove (data, exactMatch, convertTo, props) {
    ...
    if (this.values[i]) {
      ...
      _verbConsole.warn('Data ' + data + ' is not found in the tree.')
    }
